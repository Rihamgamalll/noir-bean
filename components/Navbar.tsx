...
export default function Navbar(){
 ...
 useEffect(()=>{const load=()=>fetch('/api/auth/me',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(d=>setUser(d?.user||null)).catch(()=>setUser(null));load();addEventListener('noir-auth-changed',load);return()=>removeEventListener('noir-auth-changed',load)},[]);
